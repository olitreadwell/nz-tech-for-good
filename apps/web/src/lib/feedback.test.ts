...
describe("buildEntryFeedbackUrl", () => {
  it("builds a GitHub issue URL with encoded title and body", () => {
    ...
    expect(url).toMatch(
      /^https:\/\/github\.com\/olitreadwell\/nz-tech-for-good\/issues\/new\?/,
    );
    expect(url).toContain(
      `title=${encodeURIComponent("Entry update: Test Organisation")}`,
    );
    expect(url).toContain(
      encodeURIComponent("https://nz-tech-for-good.vercel.app/entry/test-org"),
    );
  ...
  it("falls back to the slug when name is missing or blank", () => {
    ...
    expect(url).toContain(
      `title=${encodeURIComponent("Entry update: test-org")}`,
    );
