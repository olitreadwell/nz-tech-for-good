...
export default function StatsPage() {
  ...
  const withContributors = entries.filter(
    (e) => e.takes_contributors === true,
  ).length;
  const withRelated = entries.filter((e) => e.related_to.length > 0).length;

  // Founding decade distribution
  const decades: Record<string, number> = { "Before 2000": 0, "2000s": 0, "2010s": 0, "2020s": 0, Unknown: 0 };
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-8">
      ...
      <p className="mt-2 text-text-muted">
        Key numbers about the directory: entry growth, domain coverage, and data
        completeness.
      </p>
      ...
      <section className="mt-10">
        ...
        <p className="text-sm text-text-muted">
          Based on verified founding years ({entries.length - decades.Unknown} of {entries.length} known).
        </p>
            ...
            <div key={decade} className="flex items-center gap-2 text-sm">
              ...
              <div className="flex-1">
                <div
                  ...
                  style={{ width: `${Math.round((count / maxDecade) * 100)}%`, minWidth: count > 0 ? 8 : 0 }}
                />
              </div>
          ...
          {regions.slice(0, 16).map((r) => (
            <div key={r.name} className="flex items-center gap-2 text-sm">
              <span className="w-28 text-right text-text-muted truncate">
                <Link href={`/directory?region=${r.slug}`} className="hover:text-brand">{r.name}</Link>
              </span>
              <div className="flex-1">
                <div
                  className="h-5 rounded bg-brand-soft"
                  style={{ width: `${Math.round((r.count / regions[0].count) * 100)}%`, minWidth: r.count > 0 ? 8 : 0 }}
                />
              </div>
      ...
      <section className="mt-10">
        ...
        <div className="mt-3 overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              <tr className="border-b border-border"><td className="py-1.5">GitHub</td><td className="text-right">{withGithub}</td><td className="text-right text-text-muted">{entries.length - withGithub}</td></tr>
              <tr className="border-b border-border"><td className="py-1.5">LinkedIn (org)</td><td className="text-right">{withLinkedin}</td><td className="text-right text-text-muted">{entries.length - withLinkedin}</td></tr>
              <tr className="border-b border-border"><td className="py-1.5">Careers URL</td><td className="text-right">{withCareers}</td><td className="text-right text-text-muted">{entries.length - withCareers}</td></tr>
              <tr className="border-b border-border"><td className="py-1.5">Community URL</td><td className="text-right">{withCommunity}</td><td className="text-right text-text-muted">{entries.length - withCommunity}</td></tr>
              <tr className="border-b border-border"><td className="py-1.5">Events URL</td><td className="text-right">{withEvents}</td><td className="text-right text-text-muted">{entries.length - withEvents}</td></tr>
              <tr className="border-b border-border"><td className="py-1.5">Takes contributors</td><td className="text-right">{withContributors}</td><td className="text-right text-text-muted">{entries.length - withContributors}</td></tr>
              <tr><td className="py-1.5">Cross-linked</td><td className="text-right">{withRelated}</td><td className="text-right text-text-muted">{entries.length - withRelated}</td></tr>
            </tbody>
          </table>
        </div>
      </section>
