...
export default function StatsPage() {
  ...
  const mature = entries.filter(
    (e) => e.founding_year && e.founding_year < 2010,
  ).length;
  const growing = entries.filter(
    (e) => e.founding_year && e.founding_year >= 2010 && e.founding_year < 2020,
  ).length;
  const emerging = entries.filter(
    (e) => e.founding_year && e.founding_year >= 2020,
  ).length;
    ...
    <main className="mx-auto max-w-5xl px-4 py-8">
      ...
      <section className="mt-10">
        ...
        <p className="text-sm text-text-muted">
          Based on verified founding years ({entries.length - decades.Unknown}{" "}
          of {entries.length} known).
        </p>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              <tr className="border-b border-border">
                ...
                <td className="text-right text-text-muted">
                  {entries.length - withContributors}
                </td>
              </tr>
              <tr>
