...
export async function GET() {
  ...
  const urls = [
    ...
    ...domains.map((d) => `<url><loc>${base}/domains/${d.slug}</loc><priority>0.6</priority></url>`),
    ...entries.map((e) => `<url><loc>${base}/entry/${e.slug}</loc><lastmod>${e.last_verified}</lastmod><priority>0.5</priority></url>`),
  ];
